...
function renderizarNotasEmitidas() {
  ...
  notasEmitidas.forEach((item) => {
    ...
    // Envolvemos o número da nota em um span clicável diretamente no innerHTML
    const numeroNotaSpan = div.querySelector('strong'); // O strong agora contém o span
    if (
      numeroNotaSpan &&
      numeroNotaSpan.firstElementChild &&
      numeroNotaSpan.firstElementChild.classList.contains('codigo-clicavel')
    ) {
      numeroNotaSpan.firstElementChild.addEventListener('click', (e) =>
